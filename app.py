...
with tab1:
    ...
    if not grupos:
        ...
        grupo = st.selectbox("Grupo", list(grupos.keys()), key='eval_grupo')
        ...
        if not alumnos:
            ...
            # Estado del índice
            idx = st.number_input("Selecciona un alumno:", min_value=0, max_value=len(alumnos) - 1, step=1, key='eval_idx')
            alumno = alumnos[int(idx)]
            st.subheader(f"Evaluando: {alumno}")

            # Evaluación de jornada
            fecha = st.date_input("Fecha", datetime.today(), key='eval_fecha')
            hubo_teo = st.checkbox("Hubo clase teórica?", key='eval_teo')
            if hubo_teo:
                participacion = st.selectbox("Participación", ["", "Kahoot", "Mentimeter", "Apuntes"], key='eval_part')
                no_part       = st.selectbox("¿No participa?", ["", "Justificada", "Injustificada"], key='eval_nopart')
            else:
                participacion = ""
                no_part       = ""

            hubo_prac = st.checkbox("Hubo clase práctica?", key='eval_prac')
            if hubo_prac:
                material = st.selectbox("Material", ["", "Sí", "No (Justificada)", "No (Injustificada)"], key='eval_mat')
            ...
            # Guardar evaluación
            if st.button("Guardar evaluación", key='eval_guardar'):
